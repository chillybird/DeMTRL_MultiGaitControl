...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.action_means = np.zeros((self.episode_length, self.n_rollout_threads, act_shape), dtype=np.float32)
        self.action_stds = np.zeros((self.episode_length, self.n_rollout_threads, act_shape), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, action_means, action_stds, bad_masks=None, active_masks=None):
        ...
        self.action_means[self.step] = action_means.copy()
        self.action_stds[self.step] = action_stds.copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        action_means = self.action_means.reshape(-1, self.action_means.shape[-1])
        action_stds = self.action_stds.reshape(-1, self.action_stds.shape[-1])
        ...
        for indices in sampler:
            ...
            action_means_batch = action_means[indices]
            action_stds_batch = action_stds[indices]
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                  action_means_batch, action_stds_batch,  value_preds_batch, return_batch, masks_batch, \
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                action_means_batch.append(self.action_means[:, ind])
                action_stds_batch.append(self.action_stds[:, ind])
            ...
            action_means_batch = np.stack(action_means_batch, 1)
            action_stds_batch = np.stack(action_stds_batch, 1)
            ...
            action_means_batch = _flatten(T, N, action_means_batch)
            action_stds_batch = _flatten(T, N, action_stds_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        action_means = _cast(self.action_means)
        action_stds = _cast(self.action_stds)
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                action_means_batch.append(action_means[ind:ind + data_chunk_length])
                action_stds_batch.append(action_stds[ind:ind + data_chunk_length])
            ...
            action_means_batch = np.stack(actions_batch)
            action_stds_batch = np.stack(actions_batch)
            ...
            action_means_batch = _flatten(L, N, action_means_batch)
            action_stds_batch = _flatten(L, N, action_stds_batch)
