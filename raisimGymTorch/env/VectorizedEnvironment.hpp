...
namespace raisim {
    ...
    public:
        ...
        void init() {
            ...
            critic_obDim_ = environments_[0]->getCriticObDim();
            actor_obDim_ = environments_[0]->getActorObDim();
        ...
        // define my methods
        void change_gait(int gait) {
            for (auto env: environments_)
                env->change_gait(gait);
        }
...
#pragma omp parallel for schedule(auto)
        ...
        void get_phase(Eigen::Ref<EigenRowMajorMat> &phase) {
#pragma omp parallel for schedule(auto)
            for (int i = 0; i < num_envs_; i++)
                environments_[i]->get_phase(phase.row(i));
        }

        void get_velocity(Eigen::Ref<EigenVec> &vels) {
#pragma omp parallel for schedule(auto)
            for (int i = 0; i < num_envs_; i++)
                vels[i] = environments_[i]->get_velocity();
        }


        void step(Eigen::Ref<EigenRowMajorMat> &action,
                  Eigen::Ref<EigenVec> &reward,
                  Eigen::Ref<EigenBoolVec> &done, bool last_action=false) {
#pragma omp parallel for schedule(auto)
            for (int i = 0; i < num_envs_; i++)
                perAgentStep(i, action, reward, done, last_action);
        }

//        void ma_step(const Eigen::Ref<EigenVecMat>& action,
//                     Eigen::Ref<EigenRowMajorMat> &reward,
//                     Eigen::Ref<MatrixBoolXd> &done) {
//            std::cout << "sucess." << std::endl;
//        }

        void ma_reset() {
            std::cout << "ma reset." << std::endl;
        }

        int getCriticObDim() {
            return critic_obDim_;
        }
...
#pragma omp parallel for
        ...
        void position_logging(Eigen::Ref<EigenRowMajorMat> &rewards) {
#pragma omp parallel for
            for (int i = 0; i < num_envs_; i++)
                environments_[i]->position_logging(rewards.row(i));
        }

        void contact_logging(Eigen::Ref<EigenRowMajorBoolMat> &contacts) {
#pragma omp parallel for
            for (int i = 0; i < num_envs_; i++)
...
#pragma omp parallel for schedule(auto)
        ...
        inline void perAgentStep(int agentId,
            ...
            reward[agentId] = environments_[agentId]->step(action.row(agentId), last_action);
        ...
        int num_envs_ = 1;
        int obDim_ = 0, actionDim_ = 0, critic_obDim_=0, actor_obDim_=0;
